...
class Encounter:
    def __init__(self, description, action, probability):
...
def fall_encounter(player):
    fancy_prent("You slip while climbing the mountain and lose 20 energy!")
    player.energy -= 20

    
class Location:
    def __init__(self, name, description, food_chance, items, requires=None, encounter=None):
        self.name = name
        self.description = description
        ...
        if new_location in self.location.neighbors:
            ...
            if self.energy >= cost:
                ...
                if not self.location.visited:
                    ...
                    # add encounter call here based on what location you are in
